...
class SearchNet3(nn.Module):
    ...
    def __init__(self, layers, depth, connections, cell_arch1, cell, dataset, num_classes, base_multiplier=40):
        '''
        Args:
                ...
                for connection in self.connections:
                    if ([i, j] == connection[1]).all():
                        ...
                        if connection[0][0] == -1:
                            ...
                            self.cells[i][j][str(connection[0])] = cell(
                    ...
                    if ([i, j] == connection[1]).all():
                        if connection[0][0] == -1:
                            ...
                            features[i][j] += self.cells[i][j][str(connection[0])](
                                features[connection[0][0]][connection[0][1]], normalized_alphas[i][j])
                        k += 1
